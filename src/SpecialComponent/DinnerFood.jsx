...
import "../SpecialComponent/Dinner.css";
...
const DinnerFood = ({dinner}) => {
  ...
  const {id,name,price,rate,img} = dinner
  const {dispatch} = useContext(CartContext)
  const [newitem,setItem] = useState({dinner,id:crypto.randomUUID(1),name,img,price,rate})
  const handleAdd = (newItem)=>{
   setItem({
     ...dinner
   })
   if(dinner.id === newitem.id){
